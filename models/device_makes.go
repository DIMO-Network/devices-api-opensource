...
var DeviceMakeWhere = struct {
	...
	LogoURL:         whereHelpernull_String{field: "\"devices_api\".\"device_makes\".\"logo_url\""},
	OemPlatformName: whereHelpernull_String{field: "\"devices_api\".\"device_makes\".\"oem_platform_name\""},
}
...
var (
	deviceMakeAllColumns            = []string{"id", "name", "external_ids", "created_at", "updated_at", "token_id", "logo_url", "oem_platform_name"}
...
func (o *DeviceMake) DeviceDefinitions(mods ...qm.QueryMod) deviceDefinitionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"devices_api\".\"device_definitions\".\"device_make_id\"=?", o.ID),
	)
...
func (deviceMakeL) LoadDeviceDefinitions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDeviceMake interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`devices_api.device_definitions.device_make_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func DeviceMakes(mods ...qm.QueryMod) deviceMakeQuery {
	mods = append(mods, qm.From("\"devices_api\".\"device_makes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"devices_api\".\"device_makes\".*"})
	}
...
func FindDeviceMake(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*DeviceMake, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"devices_api\".\"device_makes\" where \"id\"=$1", sel,
	)
...
func (o *DeviceMake) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"devices_api\".\"device_makes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"devices_api\".\"device_makes\" %sDEFAULT VALUES%s"
		}
...
func (o *DeviceMake) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"devices_api\".\"device_makes\" SET %s WHERE %s",
...
func (o DeviceMakeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"devices_api\".\"device_makes\" SET %s WHERE %s",
...
func (o *DeviceMake) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"devices_api\".\"device_makes\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *DeviceMake) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"devices_api\".\"device_makes\" WHERE \"id\"=$1"
...
func (o DeviceMakeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"devices_api\".\"device_makes\" WHERE " +
...
func (o *DeviceMakeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"devices_api\".\"device_makes\".* FROM \"devices_api\".\"device_makes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, deviceMakePrimaryKeyColumns, len(*o))
...
func DeviceMakeExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"devices_api\".\"device_makes\" where \"id\"=$1 limit 1)"
