...
func (r *integrationR) GetUserDeviceAPIIntegrations() UserDeviceAPIIntegrationSlice {
	if r == nil {
		return nil
	}
...
func (r *integrationR) GetUserDeviceData() UserDeviceDatumSlice {
	if r == nil {
		return nil
	}
...
func (o *Integration) UserDeviceAPIIntegrations(mods ...qm.QueryMod) userDeviceAPIIntegrationQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"devices_api\".\"user_device_api_integrations\".\"integration_id\"=?", o.ID),
	)
...
func (o *Integration) UserDeviceData(mods ...qm.QueryMod) userDeviceDatumQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"devices_api\".\"user_device_data\".\"integration_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (integrationL) LoadUserDeviceAPIIntegrations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeIntegration interface{}, mods queries.Applicator) error {
	var slice []*Integration
	var object *Integration
	...
	query := NewQuery(
		...
		qm.WhereIn(`devices_api.user_device_api_integrations.integration_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (integrationL) LoadUserDeviceData(ctx context.Context, e boil.ContextExecutor, singular bool, maybeIntegration interface{}, mods queries.Applicator) error {
	var slice []*Integration
	var object *Integration
	...
	query := NewQuery(
		...
		qm.WhereIn(`devices_api.user_device_data.integration_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Integration appropriately.
func (o *Integration) AddUserDeviceAPIIntegrations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserDeviceAPIIntegration) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.IntegrationID = o.ID
...
// Sets related.R.Integration appropriately.
func (o *Integration) AddUserDeviceData(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserDeviceDatum) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.IntegrationID = o.ID
...
func Integrations(mods ...qm.QueryMod) integrationQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"devices_api\".\"integrations\".*"})
	}
