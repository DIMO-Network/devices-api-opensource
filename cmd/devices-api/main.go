...
func startWebAPI(logger zerolog.Logger, settings *config.Settings, pdb database.DbStore) {
	...
	taskSvc := services.NewTaskService(settings, pdb.DBS, ddSvc)
	...
	v1.Post("/user/devices/:user_device_id/commands/refresh", jwtAuth, userDeviceControllers.RefreshUserDeviceStatus)
